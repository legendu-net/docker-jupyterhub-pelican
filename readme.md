...
docker run -d --init \
    ...
    dclong/jupyterhub-pelican /scripts/sys/init.sh
```
Use the image with the `next` tag (which is the testing/next version of dclong/jupyterhub-pelican).
```
docker run -d --init \
    --hostname jupyterhub-pelican \
    --log-opt max-size=50m \
    ...
    -v "$(pwd)":/workdir \
    -v "$(dirname $HOME)":/home_host \
    dclong/jupyterhub-pelican:next /scripts/sys/init.sh
```
